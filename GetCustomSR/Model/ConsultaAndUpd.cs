...
namespace GetCustomSR.Model
{
    public class ConsultaAndUpd
    {
        public string getOrderCustomCS(string id_pedido, string idpedidorelacion)
        {
            ...
            try
            {
                var dt = cn.TraeDataTable("SELECT COUNT(*) as cont FROM m_searsorderitem WHERE sr_id =@id_pedido AND idpedidorelacion=@idpedidorelacion ", new[]
                {
        ...
        public string updOrderCustomCS(string id_pedido, string idpedidorelacion)
        {
            ...
            try
            {
                cn.EjecutaConsulta("UPDATE m_searsorderitem SET estatus ='Embarcado', mensajeria ='custom', guia ='custom' WHERE sr_id=@id_pedido AND idpedidorelacion =@idpedidorelacion", new[]
                {
                    new MySqlParameter("@id_pedido", id_pedido),
                    new MySqlParameter("@idpedidorelacion", idpedidorelacion)
                }, System.Data.CommandType.Text);
