...
using UpdStatusPendienteSR.Services;
...
namespace UpdStatusPendienteCS.Model
{
    public class EndPoint
    {
        ...
        public string UpdateSR (string idpedido, string estatus, string mensajeria, string guia,string estatus_guia)
        {
            var log = new clsLog();
            var cn = new clsConexion();
            try
            {
                cn.EjecutaConsulta("UPDATE m_searsorderitem SET estatus = @estatus , mensajeria =@mensajeria , guia = @guia, estatus_guia = @estatus_guia WHERE idpedidorelacion= @idpedido", new[]
                {
