...
namespace UpdTrackingNumberCS.Model
{
    public class EndPoint
    {
        ...
        public GetPendientes nopedido(string Nopedido)
        {
            ...
            try
            {
                var client = new RestClient(ENDPOINT + "pedidos?action=detallepedido&nopedido=" + Nopedido + "&canal=CS");
        ...
        public string UpdCSOrders(string idpedido, string tracking, string estatuspedido)
        {
            var log = new clsLog();
            var cn = new clsConexion ();
            try
            {
                cn.EjecutaConsulta("UPDATE m_claroshoporderitem SET estatus_guia = @estatuspedido, guia = @tracking, WHERE idpedidorelacion =@idpedido", new[]
                {
